...
// Controller: Check available directions (pendekat) untuk simpang
exports.checkAvailableDirections = async (req, res) => {
  ...
  try {
    // Ambil distinct dari_arah (pendekat) saja
    const [rows] = await connection.query(
      `SELECT DISTINCT dari_arah AS arah
       FROM arus FORCE INDEX (idx_arus_simpang_dari)
       WHERE ID_Simpang = ?`,
      [simpangId]
    );
    ...
    if (!rows || rows.length === 0) {
      return res.status(404).send({
        success: false,
        message: `Tidak ada data arah untuk simpang dengan id ${simpangId}`
      });
    }

    // Mapping English ke UTBS
    const directionMap = {
      'north': 'u',
      'east': 't',
      'south': 's',
    ...
    const directions = [...new Set(rows.map(r => directionMap[r.arah.toLowerCase()] || r.arah))];

    res.send({
      success: true,
      simpangId,
      directions // contoh: ["t","s","b"]
    });

  } catch (error) {
    console.error('Error in checkAvailableDirections:', error);
    res.status(500).send({
      success: false,
      message: error.message || "Terjadi error saat cek arah simpang."
    });
  } finally {
